...
public class Op {
    ...
    public static void ClickMsg(int x, int y, string button, int clicks) {
        SetCursorPos(x, y);
    ...
    try {
        ...
        switch ($cmd) {
            ...
            "move" {
                ...
                # Glide, not SetCursorPos: a bare move is usually there to
                # trigger a hover, and hover menus need the mousemove events.
                [Op]::Glide($p[0], $p[1])
                Reply @{ ok = $true }
            }
            ...
            "click" {
                ...
                if ($script:Hidden) { [Op]::ClickMsg($p[0], $p[1], $btn, $n) }
            ...
            "scroll" {
                ...
                [Op]::Scroll($p[0], $p[1], $amt, $h)
                Reply @{ ok = $true }
            }

            "type" {
                if ($script:Hidden) { [Op]::TypeMsg("$($req.text)") }
                else { [Op]::TypeText("$($req.text)") }
                Reply @{ ok = $true }
            }

            "key" {
                $err = if ($script:Hidden) { [Op]::KeyMsg("$($req.keys)") } else { [Op]::PressCombo("$($req.keys)") }
